...
observeEvent(input$upload_counts, {
  ...
  # check the correspondance to coldata
  validate(need(all(my_values$coldata[, "names"] == colnames(my_values$counts)),
                "Columns names don't match with sample table \"names\" column"))
})
...
observeEvent(input$import, {
  ...
  validate(need(file.exists(my_values$coldata$files),
               message = "Some files specified by sample table names do not exist"))
